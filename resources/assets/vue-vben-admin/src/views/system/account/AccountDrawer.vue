...
<script lang="ts" setup name="AccountDrawer">
  ...
  const id = ref(0);
  const roleRouteTreeData = ref<Recordable[]>([]);
  ...
  const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
    ...
    if (unref(isUpdate)) {
      ...
      roleRouteTreeData.value = await getRoleRoute(account.roles);
    } else {
      id.value = 0;
    }
    ...
    updateSchema([
        ...
        componentProps: ({ formModel, formActionType }) => {
          return {
            ...
            onChange: async (e: any) => {
              formModel.home_path = null;
              const route = await getRoleRoute(e);
              const { updateSchema } = formActionType;
              updateSchema({
                field: 'home_path',
                componentProps: {
                  treeData: route.items,
                },
              });
            },
          };
      ...
      {
        field: 'home_path',
        componentProps: { treeData: roleRouteTreeData.value },
      },
    ]);
  });
