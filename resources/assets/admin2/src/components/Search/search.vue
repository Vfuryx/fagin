  ...
  <div class="admin_table_page_where" v-if="show">
    <a-form layout="inline">
      <a-row :gutter="[24,24]">
        <a-col :md="6" :sm="24" v-for="(v,k) in searchConfig1" :key="k">
          ...
          >
            ...
            <a-select
              ...
              :placeholder="v.placeholder||''">
              <a-select-option
                v-for="(vo,key) in v.data"
                :key="key"
                :value="vo.value">{{ vo.label }}
              </a-select-option>
            </a-select>
            <a-time-picker
              v-if="v.type==='time_picker'"
              v-model="params[v.name]" />
            <a-range-picker
              style="width:100%"
              v-if="v.type==='date_picker'"
              v-model="params[v.name]"
              format="YYYY-MM-DD HH:mm:ss"
              value-format="YYYY-MM-DD HH:mm:ss"
              show-time
              :allow-clear="false"></a-range-picker>
          </a-form-item>
        </a-col>
          ...
          <a-col :md="6" :sm="24" v-for="(v,k) in searchConfig2" :key="k">
            ...
            >
              ...
              <a-select v-if="v.type==='select'" v-model="params[v.name]" :placeholder="v.placeholder||''">
                <a-select-option v-for="(vo,key) in v.data" :key="key" :value="vo.value">{{ vo.label }}
                </a-select-option>
              </a-select>
        ...
        <a-col :md="!advanced && 6 || 24" :sm="24" style="padding-top:16px;">
          <span
...
export default {
  ...
  created () {
    ...
    let autoParamsArr = []
    if (this.autoParams !== undefined) {
      autoParamsArr = Object.keys(this.autoParams)
    }
