<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="list" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="分组名称" prop="name" :show-overflow-tooltip="true" width="150"/>
...
<script>
import {gAdd, gList, gShow, gUpdate} from '@/api/permission'
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.id !== undefined) {
            gUpdate(this.form).then(response => {
              if (response.code === 0) {
                // 提示信息
                this.$notify({
                  title: '成功',
