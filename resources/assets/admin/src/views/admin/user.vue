...
<script>
// import { listUser, , exportUser, resetUserPwd, changeUserStatus, importTemplate, getUserInit } from '@/api/user'
import { listUser, userRolesList, addUser, getUser, updateUser, changeUserStatus, delUser, delUsers, resetUserPwd } from '@/api/user'
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除用户编号为"' + userIds + '"的数据项?', '警告', {
        ...
        if (isNaN(userIds)) {
          const arr = [1]
          const difference = userIds.filter(x => !arr.includes(x))
          return delUsers(difference)
        } else {
